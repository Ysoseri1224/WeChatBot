...
- **文件管理**：微信缓存原始文件 + 已转换文件 + 转换队列（手动触发走暂存→确认/取消流程，避免污染 `memory/files/`）
...
#### 转换队列 + WebUI 视觉重构 + 日志 bug 修复

修掉一个隐蔽的致命 bug：`_SseLogHandler.emit` 里用 `self.formatTime(...)` 生成时间字符串，但 `logging.Handler` 根本没有这个方法，调用持续抛异常又被 try/except 吞掉，结果就是实时日志页空白。改用 `datetime.fromtimestamp(record.created).strftime(...)` 之后 SSE 推流立刻恢复。

文件转换流程重构为队列 + 暂存 + 确认机制。过去在 WebUI 点「转化为 md」会直接把 Markdown 丢进 `memory/files/`，不好的转换结果会污染长期记忆。现在新增 `memory/files/.pending/` 暂存目录，手动触发的转换先落到暂存区，文件管理页多一个「转换队列」Tab，带状态徽章（正在转化/转化成功/转化失败）和确认/取消按钮，用户核对内容后再一键归档，失败的也能一键清理。对应新增三个接口：`GET /api/convert/queue`、`POST /api/convert/confirm`、`POST /api/convert/cancel`。后台文件监控线程行为不变，仍然自动归档到 `memory/files/`，保证群内读文件指令不受影响。

WebUI 样式整体重写。所有卡片、队列项、日志框、模态框都加上带 `color-mix` 的 accent 渐变背景，换绿色预设时整页视觉会跟着偏移而不是只动几个按钮。动画分三层：页面切换 fade-in、卡片 hover 抬升、队列项 slide-in 作为交互层；全局背景做 22s 缓动漂移作为氛围层；所有动效在 `prefers-reduced-motion` 下自动降级。布局上加了移动端抽屉式侧栏与汉堡按钮，断点覆盖 1100 / 900 / 640px，窄屏下 stat 改两列、队列项竖排、模态框自适应宽度。
